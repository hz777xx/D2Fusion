...
if __name__ == "__main__":
    ...
    Y = torch.rand(size=(1, 64, 456, 678), dtype=torch.float32).cuda()
    Z = torch.rand(size=(1, 64, 456, 678), dtype=torch.float32).cuda()
    #modelD=VSSBlock(hidden_dim=64,drop_path=0.1,convFFN=True).cuda()
    modelD = RSSBlock(hidden_dim=64, drop_path=0.1, attn_drop_rate=0.1, d_state=16, expand=2.0, is_light_sr=False).cuda()
    X=modelD(X,(128,128))
    X=X.transpose(1, 2).view(1, 64, 128, 128)
    print(modelD)
