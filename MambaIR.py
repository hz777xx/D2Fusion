...
class VSSBlock(nn.Module):
    def __init__(
        ...
        ####modify attention
        #self.conv_blk = CrissCrossAttention(in_dim=hidden_dim)
...
if __name__ == "__main__":
    vss_block = VSSBlock(hidden_dim=64, d_state=16, expand=2., is_light_sr=False)
    x = torch.randn(2, 256, 64)
    x_size = (16, 16)  # H=16, W=16
    output = vss_block(x, x_size)
    print(f"VSSBlock输出形状: {output.shape}")
